...
- duplicate canceled order IDs in a one-order cancel response,
- extra `not_canceled` order IDs in a one-order cancel response,
...
- `cargo test --offline cancel`: PASS, 15 lib tests.
...
- `cargo test --offline`: PASS, 189 lib tests and 7 main tests.
