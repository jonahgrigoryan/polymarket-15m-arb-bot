...
- Scope: inert Live Alpha config defaults, Live Alpha gate evaluation, compile-time feature scaffold default-off, execution intent shape, append-only live journal with redaction/replay/reducers, live balance tracker, live position book, reconciliation engine, and reconciliation-health metrics.
...
- Reconciliation mismatch fixtures halt fail-closed for unknown open order, missing venue order, unexpected fill, filled venue order without a matching local trade order, unexpected partial fill, cancel not confirmed, reserved balance mismatch, balance delta mismatch, position mismatch, trade status failed, and same-source SDK/Rust disagreement.
- LA1 checks passed: focused LA1 filters, `cargo run --offline -- --config config/default.toml validate --local-only`, `cargo fmt --check`, `cargo test --offline` (252 lib tests + 8 main tests), `cargo clippy --offline -- -D warnings`, `git diff --check`, and safety/no-secret scans.
