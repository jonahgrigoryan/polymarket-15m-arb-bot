...
- Workspace state: final live-trading planning docs are merged to `main` via PR #42. The active work is LT0 approval and scope lock only. No live orders, cap reset, taker expansion, production sizing, multi-wallet deployment, asset expansion, cancel-all runtime behavior, secret material change, or live-trading source/config implementation is authorized from the current state.
...
- Next exit gate: complete LT0 approval/scope lock with `verification/YYYY-MM-DD-live-trading-lt0-scope-lock.md`, record open decisions and approvals, update `STATUS.md` to point to LT1, and stop for human review. Do not add live-capable source/config code, increase size, order rate, asset coverage, taker usage, runtime duration, multi-wallet deployment, or production rollout from LT0. LT0 approval remains scope-lock only and does not approve any order placement.
