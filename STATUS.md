...
- Workspace state: LT3 final-live auth, secret-handle validation, and signing dry-run support are implemented on PR #46. The branch adds final-live secret handle/account binding config, a redacted `live-trading-signing-dry-run` CLI, LT3 signing artifact schemas, redacted local artifacts under `artifacts/live_trading/LT3-LOCAL-DRY-RUN/`, and verification note `verification/2026-05-13-live-trading-lt3-auth-signing-dry-run.md`. Review hardening now blocks enabled final-live configs with `approved_authenticated_readback_not_passed` unless exact approved host/country/region scope allows authenticated readback, explicit `live_trading.legal_access_approved=true` is present, final-live wallet/funder addresses pass local nonzero EVM validation, and authenticated readback passes; the approved-host identity is read through `libc::gethostname` rather than `HOSTNAME`/`HOST` or PATH-resolved hostname binaries. `POLY_1271` remains scoped to final-live LT3 readback and is not accepted by the shared legacy Live Beta/Live Alpha signature parser. LT3 artifacts now separately report order-submit auth headers and readback auth headers, so approved-host readback can be audited without implying an order-submit header path. Local `LT3-LOCAL-*` approval ids and non-LT3 approval ids are blocked from passing enabled final-live artifacts and skip authenticated readback before any readback auth headers or live CLOB GETs are generated. Empty or whitespace-only final-live env secret values now count as missing. No live order submission, cancel submission, heartbeat POST, cap sentinel write, taker expansion, production sizing, multi-wallet deployment, asset expansion, cancel-all runtime behavior, authenticated write client, raw signature generation, order-submit auth header generation, or secret material logging is authorized or implemented.
...
- Active milestone: LT3 Auth, Secret Handles, And Signing Dry-Run is locally complete and under PR #46 review. `LIVE_TRADING_PRD.md` defines the product boundary as a gated path toward the original 1,000+ matched fills/day objective. `LIVE_TRADING_IMPLEMENTATION_PLAN.md` decomposes the LT0-LT14 sequence, including throughput readiness, staged volume ramps, and a 1,000+ fills/day candidate. LT3 adds final-live auth/secret handle validation and sanitized signing dry-run artifacts only; the approved-host command can pass only with a non-local LT3 post-approval id after explicit final-live legal/access approval, final-live account-binding validation, and final-live authenticated readback pass, and LT3 does not authorize order placement, cancel placement, raw signing output, auth header output, or a submit-ready live order.
