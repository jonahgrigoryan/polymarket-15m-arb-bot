...
- Active milestone: Live Alpha LA4 shadow live executor. LA4 routes risk-reviewed paper strategy intents into persisted shadow-live decisions and comparison artifacts, but paper remains the execution path. LA4 authorizes no live orders, no live cancels, no cancel-all, no controlled fill canary, no maker autonomy, no strategy-selected live trading, and no LA5 work. Current local evidence: focused `live_executor`, `shadow_live`, `execution_intent`, and `live_risk_engine` tests pass, including PR #33 regressions for filled-long pUSD accounting, sell-side inventory-vs-pUSD checks, and notional risk reason specificity; full `cargo test --offline`, `cargo fmt --check`, `cargo clippy --offline -- -D warnings`, `git diff --check`, and required safety/no-secret scans pass; `validate --local-only` passes with run ID `18ac876ce7f96be0-6d0f-0`; the exact required runtime command `cargo run --offline -- --config config/default.toml paper --shadow-live-alpha` now passes with run ID `18ac8770bb0d6f10-6d8f-0`, 3 markets observed, 0 paper fills, 0 shadow decisions, 0 would-submit, 0 would-cancel, 0 rejected, and no live order.
