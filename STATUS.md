...
- Next exit gate: human/operator approval to start LT1 Read-Only Final-Live Supervision. Do not start LT1 approved-host commands, add live-capable source/config code, increase size, order rate, asset coverage, taker usage, runtime duration, multi-wallet deployment, or production rollout without that explicit next approval. LT0 approval remains scope-lock only and does not approve any order placement.
