...
- Workspace state: LT1 read-only final-live supervision implementation is under PR #44 review with P1/P2 fixes applied. The branch adds `live-trading-preflight --read-only --baseline-id <id>`, a disabled-by-default `[live_trading]` config block, fail-closed final-live preflight reporting, exact approved host/jurisdiction gating before authenticated L2 readback, approved-context read-only market/book/reference/predictive freshness checks, and redacted local LT1 artifacts under `artifacts/live_trading/LT1-LOCAL-DRY-RUN/`. No live orders, order signing, cancel submission, heartbeat POST, cap sentinel write, taker expansion, production sizing, multi-wallet deployment, asset expansion, cancel-all runtime behavior, or secret material change is authorized or implemented.
