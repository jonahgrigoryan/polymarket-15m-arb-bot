...
- Branch: `live-trading/lt4-maker-shadow`.
- Current base: fresh `main`/`origin/main` at `f0457e3` (`Merge pull request #46 from jonahgrigoryan/live-trading/lt3-auth-signing-dry-run`).
- Workspace state: LT4 maker shadow and approval-envelope dry-run support is implemented locally for review. The branch adds a final-live maker evaluator/report module, `live-trading-maker-canary --dry-run --approval-id <id> --approval-artifact <path>`, a complete LT4 approval-envelope candidate at `verification/2026-05-14-live-trading-lt4-approval-candidate.md`, and a redacted dry-run report at `artifacts/live_trading/LT4-LOCAL-DRY-RUN/maker_dry_run.redacted.json`. The local default-config LT4 dry-run is intentionally blocked because final-live config, approved host/jurisdiction, wallet/funder/signature binding, baseline binding, current book/reference/predictive state, heartbeat/readback state, allowance cap, and same-window paper comparison are not present in tracked defaults. No live order submission, cancel submission, signing for submit, heartbeat POST, cap write, taker submission, batch order path, cancel-all path, raw signature output, order-submit auth header output, or secret material logging is authorized or implemented. The LT3 follow-up readback source mismatch is also minimally fixed: final-live authenticated readback now sources `required_collateral_allowance_units` from `config.live_trading` rather than `config.live_beta.readback_account` while using final-live auth/account bindings.

## Milestones

- Last completed milestone: LT3 Auth, Secret Handles, And Signing Dry-Run is complete and merged to `main` via PR #46 at `f0457e3`; LT2 Final-Live Gates, Journal, And Evidence Schema is complete and merged to `main` via PR #45 at `f4f90c0`; LT1 Read-Only Final-Live Supervision is complete and merged to `main` via PR #44 at `27e3b7f`; LT0 approval/scope lock is complete and merged to `main` via PR #43 at `823f9ef`; final live-trading PRD and implementation-plan draft are merged to `main` via PR #42 at `f57979d`. LA8 scale decision report is complete and merged to `main` via PR #41 at `89ddf0c` with decision `NO-GO: lifecycle unsafe`. LA7 selective taker gate is complete and merged to `main` via PR #40 at `fd35522`. LA7 remains a tightly scoped taker gate and review milestone, not broad taker enablement. LA6 quote manager and cancel/replace remains complete and merged to `main` via PR #35 at `d5fd1d9`; LA5 maker-only micro autonomy remains complete and merged via PR #34 at `e051054`; LA4 shadow live executor remains complete and merged via PR #33 at `cfec8dd`; LA3 controlled fill canary remains complete and merged via PR #32 at `7b7f952`; LA2 heartbeat, user events, and crash recovery remains complete and merged via PR #31 at `f493c78`; LA1 gates, journal, and reconciliation foundation remains complete and merged via PR #29 at `c6f3c23`; LA0 approval and scope lock remains complete and merged via PR #28. LB7, LB6, LB4, and M9 statuses remain as previously documented.
- Active milestone: LT4 Maker Shadow And Approval-Envelope Dry-Run is locally implemented and under verification. `LIVE_TRADING_PRD.md` defines the product boundary as a gated path toward the original 1,000+ matched fills/day objective. `LIVE_TRADING_IMPLEMENTATION_PLAN.md` decomposes the LT0-LT14 sequence, including throughput readiness, staged volume ramps, and a 1,000+ fills/day candidate. LT4 adds final-live maker quote evaluation, dry-run-only approval-envelope/report generation, and fail-closed blockers for unapproved legal/access gate, missing or mismatched approved jurisdiction, missing market identifiers, stale price-times-size notional, over-cap single-order notional, paper/live shadow mismatch, missing/stale geoblock, book/reference/predictive data, post-only marketability ambiguity, unknown tick/min size, near-close market, missing baseline binding, required heartbeat freshness, unresolved-order state, and unreviewed incidents. LT4 does not authorize order placement, cancel placement, signing for submission, cap writes, taker behavior, batch orders, cancel-all, or LT5.
...
- Next exit gate: finish LT4 verification, code review, and merge/handoff only. Stop after LT4 until a human/operator explicitly approves the exact LT5 maker canary artifact. Do not run a real maker canary, live order/cancel behavior, signing-for-submit path, cap write, taker behavior, batch path, cancel-all path, increased size/rate/asset coverage, runtime expansion, multi-wallet deployment, or production rollout from this LT4 branch.
