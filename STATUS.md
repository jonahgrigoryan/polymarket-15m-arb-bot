...
- PR #20 P1 review fix: the authenticated LB4 path now also queries read-only `GET /sampling-markets` and derives venue state from live CLOB market fields instead of hardcoding `trading_enabled`. Missing, malformed, empty, non-accepting, closed, archived, or disabled sampling-market state fails closed through `venue_state_not_open`.
