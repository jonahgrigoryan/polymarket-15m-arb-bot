...
- The runtime still requires geoblock PASS, LB4 approved-host account preflight PASS, zero open orders, L2 handles present, canary private-key handle present, LB5 rollback readiness, exact LB6 single-order cancel path availability, official SDK availability, and an unused local one-order cap sentinel.
