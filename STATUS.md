...
- Next exit gate: commit LT2, push the branch, open PR to `main`, review, merge, refresh `main`, then wait for explicit approval before LT3. Do not start LT3 auth/signing dry-run work, run live order/cancel behavior, add signing for submission, add authenticated write clients, increase size, order rate, asset coverage, taker usage, runtime duration, multi-wallet deployment, or production rollout without explicit next approval.
