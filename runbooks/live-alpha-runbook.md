...
Before submission it must rerun geoblock, authenticated readback, baseline, heartbeat, inventory, reconciliation, fresh market, fresh book, fresh reference, depth, fee, slippage, worst-price, no-near-close, and live risk gates. It must reserve the one-order cap with create-new semantics before the official SDK submit call. The submit path is one BUY GTC marketable limit through `polymarket_client_sdk_v2`; it is not batch, FOK/FAK, post-only, cancel-all, or retry-after-ambiguous-submit. After submission it must immediately run authenticated readback and baseline-aware reconciliation, write `live_alpha_taker_canary_live_report.json`, and leave the cap artifact consumed even if network ambiguity or post-submit checks fail.
